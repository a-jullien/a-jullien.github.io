...
[**ElasticSearch**](http://www.elasticsearch.org/), le moteur de recherche créé par la société du même nom. Il y a un 
 certain engouement autour du paradigme du "big data" et de l'exploitation de ces données dans des environnements 
 [**NoSQL**](http://fr.wikipedia.org/wiki/NoSQL). D'où la nécessité d'avoir des outils simples et performants pour tout ce 
 qui touche à la recherche d'information.
Je vais donc expliquer dans cet article comment fonctionne ElasticSearch, et mettre en avant les avantages de son utilisation.
...
Lucene fournit en effet un puissant système basé sur un moteur d'indexation de documents et un moteur de recherche "*full-text*" sur ces index.
...
* **Index**: ElasticSearch enregistre ses données dans un ou plusieurs index, qui peuvent être comparés à un espace logique, 
ou souvent comparés à une base de données dans le monde relationnel. C'est un concept qui provient de Lucene et permet 
d'écrire et de lire depuis ses index.
...
Et voilà, votre moteur de recherche est démarré via cette instance et prêt à être utilisé. Facile, non ?
...
Il est donc possible de vérifier que le noeud ElasticSearch est en fonctionnement: 
...
Par défaut, ElasticSearch attribue le nom de cluster *elasticsearch* et un nom de noeud de manière automatique, ici *Jackson Arvad*.
...
le mécanisme de découverte de noeuds est en multicast. Si vous démarrez deux noeuds dans un même réseau sans changer 
la configuration, ils seront automatiquement liés au même cluster *elasticsearch*. Il faut bien l'avoir en tête.

#Configuration

Comme évoqué précédemment, il est important de changer la configuration par défaut d'ElasticSearch. Le fichier de 
...
<i class="fa fa-exclamation-triangle"></i> Pour ce genre d'opération, il est préférable de ne pas exposer votre cluster directement. 
C'est à votre soin de protéger votre cluster, par l'ajout d'un *reverse proxy* pour commencer...
...
Le résultat de cette requête de recherche sera de cette forme-là:
    ...
    "_source":{"text": "Mon super post de test", "title": "Mon super post", "author": "Alex KIDD"}}
    ]}
    }
    
La réponse contient effectivement un certain nombre d'informations:

* **took**: c'est simplement le temps écoulé pour exécuter la requête (en millisecond).
* **time_out**: si oui ou non la requête a expiré.
* **_shards**: comment les partitions ont été sollicitées pour cette recherche, avec le nombre de succès et d'échecs. 
...
défaut sur ce *\_score*. En d'autres termes, ElasticSearch remonte les résultats les plus pertinents. 
...
je veux lister tous les posts où le terme *test* apparait: 
...
Ou encore, je veux lister tous les posts dont le prénom de l'auteur commence par 'A': 
...
Il y a une multitude d'opérateurs disponibles mais je ne vais pas détailler toutes les fonctionnalités offertes par le langage d'interrogation. Je vous invite donc à regarder 
...
le processus du mapping. Moi-même je n'ai pas encore exploré toutes les possibilités autour de l'analyse de documents.
...
#Agrégations
Un mécanisme d'agrégation est disponible dans ElasticSearch (anciennement appelé *facets*, maintenant *deprecated* dans la dernière version) 
afin d'extraire, de grouper vos données et de les utiliser pour un traitement particulier, comme des statistiques par exemple.
...
On voit bien ici que les auteurs des posts sont mis en exergue dans le tag *aggregations* du résultat de la requête.    

#Conclusion
Le but de ce premier billet technique est de vous présenter les mécanismes d'indexation et de recherche fournis par ElasticSearch. 
Nous avons de plus en plus besoin de recherche full-text dans nos applications, et cela pour notre simple blog jusqu'à 
notre middleware distribué contenant des milliers de Teraoctets de données. Et ElasticSearch peut effectivement être une solution, 
au même titre que l'est peut-être son concurrent [**Solr**](http://lucene.apache.org/solr/), basé lui-même sur **Lucene**. 
Seulement, n'ayant pas eu l'occasion de jouer avec **Solr**, je ne peux comparer ces deux moteurs. 

En tout cas pour ElasticSearch, j'ai trouvé sa mise en place et son utilisation vraiment simples et cela peut
réellement apporter une valeur ajoutée à vos applications. De plus, l'API cliente est disponible dans la plupart des langages.
